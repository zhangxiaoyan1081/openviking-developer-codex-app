...
文字模式执行示例结构：`python3 <实际脚本路径> <action> < <本次私有JSON文件>`。先生成 JSON 文件，不在 shell 拼接用户内容。已有规则仍需展示本次协作方式，由用户选择沿用或调整；同一次接入已确认后无需重复提问；新规则的具体内容和影响必须已展示。不得靠手改状态文件绕过校验。
...
独立接续：仅用户选择新任务时 create_thread，随后 wait_threads；结果只读核对后反馈到原任务。若子任务工具缺失，记录未验证及恢复动作；可回顾当前任务的进展，等待用户选择推进方向后再执行工作，不把本地摘要当成已经从云端取回的证明。

桌面卡片验收独立于脚本流程：连接、协作、范围、清单、整理和摘要均须有实际 App 展示证据；HIL、Markdown 和工具 JSON 不计作卡片。用户在卡片确认清单后，原卡片显示并轮询进度，不重复 show_onboarding；文字确认且没有进度卡时才展示一次。摘要通过 publish_work 单独展示一次，原进度卡保持同步状态，摘要卡不混排同步清单；脚本发布后必须调用展示工具。prepare_collaboration、publish_work、publish_report 自带 UI 资源，避免漏掉第二次展示调用。

点击“回顾这项工作”（含旧版“继续这项工作”）仅授权读取、总结与建议。概括任务目标、已完成、当前进展和待解决事项，建议几个推进方向，等待用户下一条指令；不得自动执行旧待办或为回顾额外生成文件。
